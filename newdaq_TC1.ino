// new dc offset 6/11/2025, bug fix 10/24/2025
...
// with a 16 MHz AVR clock, the ADC rate is 9615.4 sps
...
#define FILTER_ALPHA (0.001)  //time constant about 60 seconds at 18 SPS

// DC offset variable
float dc_offset = 0.0; //value of DC offset
long result = 0;
...
void initADC() {
  ...
  // ACME off, free-running mode, 75 Hz
  ADCSRB = 0;
...
ISR(ADC_vect) {
  static uint8_t runningcount = 0;
  static unsigned long runningsum = 0;
  ...
  if (++runningcount == 128) {
    next_sample =
      (unsigned int) (runningsum >> 1);  //oversample and average to 16 bit result
    runningsum = 0;
    runningcount = 0;
...
void setup() {
  ...
  //  estimate DC offset, average a few samples

  int32_t sum = 0, offset_count = 0;
  for (int i = 0; i < 6; i++) {
    if (i > 1) { //skip 1st two
      offset_count++;
...
void loop() {
  ...
  unsigned long output_sample[2] = {0}; //array for output (second) downsample step

  long filtered_signal = 0;

  // cascade of decimating low pass filters
  // https://www.musicdsp.org/en/latest/Filters/214-fast-downsampling-with-antialiasing.html
  // Each source sample is convolved with { 0.25, 0.5, 0.25 } before downsampling.

  while (!next_sample_ready);  //wait for 1st sample
  next_sample_ready = false;
  output_sample[0] = filter_state[0] + (next_sample >> 1);

  while (!next_sample_ready);  //wait for 2nd sample
  next_sample_ready = false;
  filter_state[0] = next_sample >> 2;
  output_sample[0] += filter_state[0];

  while (!next_sample_ready);  //wait for 3rd sample
  next_sample_ready = false;
  output_sample[1] = filter_state[0] + (next_sample >> 1);

  while (!next_sample_ready);  //wait for 4th sample
  next_sample_ready = false;
  filter_state[0] = next_sample >> 2;
  output_sample[1] += filter_state[0];

  // decimate the two outputs about and compute result

  filtered_signal = filter_state[1] + (output_sample[0] >> 1);
  filter_state[1] = output_sample[1] >> 2;
  filtered_signal += filter_state[1];

  result = filtered_signal - (long)(dc_offset + 0.5);  //corrected value

  // update low pass DC offset
  dc_offset += (filtered_signal - dc_offset) * FILTER_ALPHA;

  //Serial.print(dc_offset,0);
  //Serial.print(", ");
  if (!first) Serial.println(result); //skip printing 1st result
  else first = false;
}
